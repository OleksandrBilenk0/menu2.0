function getRandomBool() {
  ...
  function onEvent(x, y) { 
    ...
    game.draw();
    if (game.victory()) {
      alert("Складено за "+game.getClicks()+" натискань!"); 
      game.mix(300);
      context.fillRect(0, 0, canvas.width, canvas.height);
  ...
  gameBtn.addEventListener('click', showGameInfo);
  ...
  function toggleGameInfo() {
    if (isGameInfoOpen) {
      gameContent.style.display = 'none'; // Скрываем информацию
    } else {
      gameContent.style.display = 'block'; // Отображаем информацию
      showGameInfo(); // Показываем игру при открытии
    }
    isGameInfoOpen = !isGameInfoOpen; // Инвертируем флаг
  }
  
  // Добавляем обработчик события на кнопку
  gameBtn.addEventListener('click', toggleGameInfo);
